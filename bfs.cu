...
int main(int argc, char *argv[]) {
    ...
    // below is used to handle the command line arguments in terminal and places thw value corresponding to called argument
    ...
    // cudaEventCreate is repsonsible for creating CUDA event. It takes a pointer to 'cudaEvent_t' variable and initializes it with newly craeted object 
    // checkCudaErrors is not in CUDA API, wrapper used to handle error if generated by cuda program
    checkCudaErrors(cudaEventCreate(&start));
    checkCudaErrors(cudaEventCreate(&end));

    // vertex file name (to change)
    ...
    file.read((char*)(&vertex_count), 8);   // to change
    file.read((char*)(&typeT), 8);          // to change


    // vertex count tells total no of vertices
    vertex_count--;
    ...
    file.read((char*)(&edge_count), 8); // to change
    file.read((char*)(&typeT), 8);      // to change
    ...
    // 'mem' is command argument indicating GPUMEM, UVM_READONLY, UVM_DIRECT
    switch (mem) {
        case GPUMEM:
        ...
        case UVM_READONLY:
            ...
            // cudaMemAdviseSetReadMostly Data will mostly be read and only occassionally be written to
